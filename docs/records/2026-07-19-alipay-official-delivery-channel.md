...
### 新事实(后于 2026-07-20 反转, 本文只记事实, 决策见 dropped-decision 文档)
- **支付宝官方 `delivery.send` 接口存在 + 支持虚拟商品**: 发货后支付宝订单系统内自带"何时发货"(ship_done_time)、"已履约"(DELIVERED)、"已结算"权威留痕
- **支付宝结算记录绑定履约事实**: 若能用则比自架链证更权威,且是支付宝系统内可被小二核查的官方证据
- **但对 Savvy 场景不适用**: delivery.send 只认 `KX_SHOPPING`/`VIRTUAL_RECHARGE` 两 bizType, Savvy"SaaS 充值跑自己模型"应落软件开发服务类目, 不在认列; 强开虚拟充值类踩合规雷。→ 详见 `2026-07-19-antchain-evidence-dropped-decision.md` 文末「最终决策(2026-07-20 反转)」: 路线 B 否决, 回自架链证。本文档保留作"查过什么"的留痕, 不代表当前要走官方履约链路。
