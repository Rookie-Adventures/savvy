...
description: "触发：用户提到充值/买额度/查余额/查用量/查订单/兑换码/退款申请，或说『额度不够了』『还剩多少』『刚才那笔到账没』『这笔退一下』时调用。输入：用户意图与必要参数（充值场景为用户自定义的金额，其他场景为用户 API Key 或订单号）。行为：先按意图路由——充值/查余额/查用量/查订单/兑换码/退款申请；充值按金额与插件能力选择微信 AI 支付（对话内授权即付）或原生微信扫码（代触发 Native 订单），其余意图调用对应能力层接口并用用户身份（Authorization: Bearer <access_token>）鉴权。输出：人话结果；充值成功返回到账确认（游客另含 claim_token 与认领直达链接），退款返回工单号。边界：金额、频次、单笔上限一律由服务端裁定——接口拒绝就把错误原文原样转述，本文档不重复服务端已经把握的数字；严禁虚构二维码/支付链接/到账状态；涉及资产变动的一步操作（兑换、退款申请）必须先征得用户确认；同一订单只入账一次（幂等）。"
license: MIT
...
tags:
  ...
  - Savvy
  - 额度管家
changelog: "2.0.1 金额上下限不再写死(改由服务端裁定,避免后端放开后技能还在拦);description 补触发词;路径 A 非 402 一律降级到路径 B(含 503 SKILLPAY_DISABLED);A-4 明确付款码不符返回 401;2.0.0 多意图能力层：查余额/用量/订单、优惠码兑换、退款申请(工单)、低余额续费提醒、认领后引导，统一走用户 API Key 鉴权；1.2.4 认领链接带 claim_token；1.2.3 工具优先(createWechatTopUp/queryTopUpStatus)；1.2.2 修正接口路径前缀(/api/user/agent/...)；1.2.1 防幻觉铁律 + X-Agent-Token 鉴权说明"
---
...
金额（元）**由用户指定**，本 Skill **不掌握也不复述金额上下限**——裁定权在服务端（Native 下单的最小额、单笔上限都是服务端配置，随时可变）。本 Skill 不再随服务端联动修改，避免了"后端放开、技能还在拦"的漂移。

- 用户没说金额 → **先问**，确认后再发起
- 接口因金额拒绝 → **原样转述错误原文**（如"充值金额需在 X~Y 元之间"），不要自行替用户改金额，也不要编造范围
- 金额 >100 元**只能走路径 B**（X402 通道的策略限制，与服务端无关）
...
### A-2：处理响应（402 = 可支付；其他 = 降级）

**402**：保存响应头 `WeixinPay-Required`（支付码）与 `X-Out-Trade-No`（订单号），缺一不可 → 继续 A-3。

**非 402 一律降级处理，不要让对话卡住：**

| 响应 | 含义 | 动作 |
|---|---|---|
| `503 SKILLPAY_DISABLED` | 平台未开通 X402 通道 | **改走路径 B 扫码**，不向用户抱怨通道问题 |
| `500` / `ORDER_FAIL` | 下单失败（多为支付配置缺失） | 改走路径 B；仍失败才告知用户"暂时无法发起充值，请联系管理员" |
| `400` 金额类错误 | 服务端不收这个金额 | 原样转述错误原文，等用户改金额后从 A-1 重发 |

降级时不要重复 A-1 的订单（已有未支付单），但路径 B 是独立通道，可直接按 B-1 新建。
...
⚠️ `WeixinPay-Required` 必须与 A-2 拿到的完全一致：缺失或不符会返回 `401 PAYMENT_CODE_INVALID`（付款码是"已付款"的凭证，仅有订单号不算）。
