...
tags:
  ...
  - 支付
  - Savvy
changelog: "1.2.1 防幻觉铁律(禁虚构二维码/报错原样转述/未付款禁称到账) + X-Agent-Token 鉴权说明；1.2.0 双路径：≤100 元走微信 AI 支付(X402)，>100 元/无插件走原生扫码代触发"
---
...
- **接口返回 `"message": "error"` 时（如金额低于 1 元）**：必须把 `data` 里的错误信息**原样转述给用户**并停止，等用户修改后重新发起。**严禁自行编造二维码、支付链接或"已生成"的假反馈**

## 鉴权（如平台已配置环境变量）

若智能体平台为本 Skill 配置了环境变量 `AGENT_TOPUP_TOKEN`，则本 Skill 的所有请求必须携带请求头：

```
X-Agent-Token: <AGENT_TOPUP_TOKEN 的值>
```

缺少或值错误时接口返回 401，把错误转述给用户并提示联系管理员，不要自行重试。

## 防幻觉铁律（优先级最高）

1. **只有接口响应里的 `code_url` 才是真实支付二维码**，除此之外不得向用户展示任何二维码/图片/支付链接
