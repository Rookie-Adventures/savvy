...
// finishInsert runs post-creation tasks (role-based sidebar config, signup
// quota logs, inviter rewards) after the user-create transaction commits.
...
// UpdateWithTx updates the user inside an existing transaction, omitting quota,
// used_quota, and request_count so concurrent billing/distribution writes are
// not clobbered by this update. Reloads the row into the receiver on success.
